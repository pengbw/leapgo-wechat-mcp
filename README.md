...
1. **调用服务器 IP 必须加入白名单** — 微信公众平台 API 要求调用方的公网 IP 已加入白名单，否则返回 `40163` / `40001` 错误。详见下方「白名单配置」章节。
2. **`account_info` 查不到公众号头像** — 微信 API 不支持，请登录 mp.weixin.qq.com 后台查看
3. **视频无法以「视频卡片」形式插入草稿** — 必须手动在编辑器操作
4. **`thumb_media_id` 需要先上传图片获取** — 不能通过 URL 远程图片直接引用
5. **access_token 有效期 2 小时** — 已实现自动刷新，无需手动处理

---

## 🔐 白名单配置（必看）

微信公众平台 API 有 IP 白名单限制，**调用服务器的公网 IP 必须加入白名单**，否则任何 API 调用都会失败。

### 现象

如果遇到以下错误，说明服务器 IP 未在白名单中：

| 错误码 | 错误信息 | 原因 |
|--------|----------|------|
| `40163` | `illegal ip 39.106.xxx.xxx not in white list` | IP 未加入白名单 |
| `40001` | `access_token is invalid` | 通常也是 IP 问题 |
...
2. 进入 **设置与开发 → 基本配置**
3. 找到 **IP白名单**，点击「查看」
4. 点击「添加」，填入你的服务器公网 IP
5. 保存

### 多服务器场景

如果 MCP 服务部署在多台服务器，每台服务器的 IP 都需要单独加入白名单。

### 快速验证

```bash
# Linux 服务器上执行，替换 YOUR_IP 为查到的公网 IP
curl -s "https://api.weixin.qq.com/cgi-bin/getcallbackip?access_token=TEST" | grep "40163"
# 如果返回 40163 说明 IP 未在白名单中
```
